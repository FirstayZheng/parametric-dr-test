...
class UMAPGlobalLoss(UMAPLoss):
    ...
    def forward(self, embed_to_from, placeholder=None):
        ...
        non_loss = super().forward(embed_to_from)
        linear_loss = self._global_feature_loss(embed_to_from, placeholder).to(
            self.device)
        # print("non loss:", non_loss, " linear loss:", linear_loss)
        # print("warmup: ", self.warmup)
        if self.batch_count % 100 == 0:
            print("before non loss:", non_loss, " linear loss:", linear_loss)
        ...
        if alpha > 0.5 and beta != 0:
            # non linear占比大，以此为准
            if -1 * linear_loss > non_loss:
                num = torch.round(-1 * linear_loss / non_loss)
                linear_loss = linear_loss / num
            else:
                num = torch.round(non_loss / (-1 * linear_loss))
                linear_loss = linear_loss * num
        ...
        if self.batch_count % 100 == 0:
            print("num ", num)
            print("after non loss:", non_loss, " linear loss:", linear_loss)
            print("alpha beta :",  alpha, beta, "  loss:", (alpha * non_loss + beta * linear_loss))
            print("---------------------")
        return alpha * non_loss + beta * linear_loss
        # return loss_g
